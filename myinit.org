...
(defun convert-pdf-to-svg (relative-path absolute-path)
  ...
  (let* ((svg-file (concat (file-name-sans-extension relative-path) ".svg")))
    (message (format "Converting %s to %s" absolute-path svg-file))
    (unless (file-exists-p svg-file)
      (shell-command (format "inkscape --pdf-poppler -o %s %s" svg-file absolute-path)))
    svg-file))
        ...
        (when (eq (car link-data) 'link) ; Check if the context is a link
          (let* ((path (org-element-property :path link-data))
            ...
            (when (string= type "file")
              (let* ((new-file (perform-replacement path)))
...
(defun my/org-process-subfigure-dsl (backend)
...
#+begin_src emacs-lisp
;; Need to think about which order to apply this hook relative to the other
;; HTML hook that converts PDFs to SVG!
...
(defun subfigure-latex (width caption label body)
  ...
  %s
\\end{subfigure}%%"
                           width body (plist-get caption :caption) (plist-get label :label))))
...
(defun subfigure-html (width caption label body)
